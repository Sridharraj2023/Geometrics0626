  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ width: '32px', height: '2px', background: 'var(--color-brand)', margin: '10px 0' }} />
        <p
      ...
      <div className="flex flex-1 divide-x divide-gray-200">
        {CARDS.map((card) => (
          ...
          >
            ...
            {/* Text overlay — bottom, AB style */}
            <div
              className="absolute bottom-0 left-0 right-0 flex flex-col justify-end"
              style={{ padding: '20px 24px', background: 'linear-gradient(to top, rgba(0,0,0,0.72) 0%, transparent 100%)' }}
            >
              <p
                className="m-0 text-white mb-2"
                style={{ fontSize: '9.5px', letterSpacing: '0.22em', textTransform: 'uppercase', opacity: 0.5 }}
              >
                {card.tag}
              </p>
              <p
                className="m-0 text-white font-medium leading-snug mb-4"
                style={{ fontSize: '14px' }}
              >
                {card.title}
              </p>
